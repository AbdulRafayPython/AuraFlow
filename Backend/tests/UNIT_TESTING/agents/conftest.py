...
def _no_redis_claims():
    ...
    `engagement._claim_nudge`, `wellness._claim_checkin`) for unit tests.
    ...
    on the same channel/community/user. Forcing ``get_redis() -> None`` makes
    the claims deterministically fail-open regardless of the environment."""
    with ExitStack() as stack:
